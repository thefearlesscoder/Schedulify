...
# --- NEW: SOFT CONSTRAINTS (PREFERENCES) ---
TEACHER_PREFERENCES = {
    'PROF_KNUTH': {
        'preferred_rooms': ['NLHC-1', 'NLHC-2'], # Prefers large lecture halls
        'preferred_slots': [] # No time preference
    },
    'PROF_EDISON': {
        'preferred_rooms': [], # No room preference
        'preferred_slots': [t for t in TIMESLOTS if '09-10' in t or '10-11' in t] # Prefers morning slots
    },
     'PROF_WATT': {
        'preferred_rooms': ['GS-3'], # Example from your query
        'preferred_slots': []
    }
}
...
def evaluate_timetable(individual):
    ...
    The fitness is a penalty score. Lower is better.
    Hard violations have a high penalty (1000). Soft violations have a low penalty (1).
    """
    hard_violations = 0
    soft_violations = 0
    ...
    for course_id, teacher, room, timeslot in individual:
        ...
        if teacher in TEACHER_PREFERENCES:
            ...
            # S1: Teacher room preference
            if prefs['preferred_rooms'] and room not in prefs['preferred_rooms']:
                soft_violations += 1
            # S2: Teacher time preference
            if prefs['preferred_slots'] and timeslot not in prefs['preferred_slots']:
                soft_violations += 1
...
def main():
    ...
    if hof:
        ...
        print("A score of 0 means all hard constraints are met.")
        print("The decimal part of the score represents the number of soft constraints (preferences) violated.")
